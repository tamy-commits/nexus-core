...
export interface NexusDecision {
  ...
  audit: Array<{ actor: string; action: string; rule?: string; finding?: string; from_state?: string; to_state?: string }>;
}

const API_URL = import.meta.env.VITE_NEXUS_API_URL ?? "http://localhost:8000";

export async function analyzeDocumentCase(scenario: Scenario, issuedAt: string): Promise<NexusDecision> {
  const response = await fetch(`${API_URL}/v1/cases/analyze-demo`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ case_id: scenario.caseId, segment: scenario.segment, request_type: scenario.requestType, documents: scenario.documents.map((document) => ({ id: document.id, label: document.label, file: document.file, version: document.version, issued_at: document.id === "endereco" ? issuedAt : null, readable: true, present: true })) }),
  });
